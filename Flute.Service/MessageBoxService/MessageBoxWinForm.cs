...
namespace Flute.Service
{
	...
	/// Message box factory.
	/// </summary>
	public sealed class MessageBoxWinForm
	{
		private MessageBoxWinForm() { }
		/// <summary>
		/// Shows debug.
		/// </summary>
		...
		public static void Info(string title, string text, string detail)
		{
			...
			cTaskDialog.UseToolWindowOnXP = false;
		...
		public static DialogResult Confirm(string title, string text, string detail)
		{
			cTaskDialog.ForceEmulationMode = true;
			cTaskDialog.UseToolWindowOnXP = false;
			return cTaskDialog.MessageBox(title, text, detail, eTaskDialogButtons.OKCancel, eSysIcons.Question);
		}
	}
}
